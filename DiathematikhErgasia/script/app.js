...
function getShipPosition(x,y,length,cases){
  let shipPosition = []
  ...
  for(let j = i; condition; j+= increment){ 
    if(board.includes(coordsX[x]+coordsY[j]) || board.includes(coordsX[j]+coordsY[y])) offsetX == 0 ? shipPosition.push(coordsX[x]+coordsY[j]) : shipPosition.push(coordsX[j]+coordsY[y])
...
function removeSquares(shipSquares){
  shipSquares.forEach(shipsquare =>{
    ...
    if(board.includes(shipsquare)) board.splice(board.indexOf(shipsquare),1)
    squares.forEach(square => {if(board.includes(square)) board.splice(board.indexOf(square),1)}) 
  })
}
function createShip(num){  
  ...
  do{
    ...
    let startSquare = board[getRandomNum(board.length-1,0)]
    let [x,y] = [coordsX.indexOf(startSquare[0]),parseInt(startSquare[1])]; 
    shipPosition = getShipPosition(x,y,shipLength,[shipLength,-shipLength,0]) 
